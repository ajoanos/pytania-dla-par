...
if ($session) {
    ...
    if ($allFinished) {
        ...
        if (!haveParticipantsApprovedRematch($participants, $votes)) {
            respond(['ok' => false, 'error' => 'Najpierw oboje kliknijcie „Gramy jeszcze raz?”.']);
        }
    }
}
...
if (empty($positions)) {
    ...
    try {
        ...
        $deleteVotesStmt = $db->prepare('DELETE FROM tinder_rematch_votes WHERE room_id = :room_id');
