...
const rematchEndpoint = 'api/tinder_rematch.php';
...
let rematchVotes = new Map();
let rematchReady = false;
let rematchVoteSubmitting = false;
let rematchPromptedSessionId = null;
...
function hasSelfApprovedRematch() {
  return rematchVotes.get(participantNumericId) === true;
}

function countApprovedRematchVotes() {
  let total = 0;
  rematchVotes.forEach((value) => {
    if (value) {
      total += 1;
    }
  });
...
function updateRematchUi() {
  if (!playAgainButton || !summaryLead) {
    return;
  }
  if (!currentSession) {
    playAgainButton.disabled = true;
    playAgainButton.textContent = 'Gramy jeszcze raz?';
    summaryLead.textContent = 'Gdy oboje skończycie swipować, pokażemy tu listę wspólnych typów.';
    return;
  }
  ...
  const approvals = countApprovedRematchVotes();
  const selfApproved = hasSelfApprovedRematch();

  if (rematchReady) {
    playAgainButton.disabled = true;
    playAgainButton.textContent = 'Nowa runda za chwilę…';
    if (isHost) {
      summaryLead.textContent = 'Wybraliśmy wszystkie wspólne typy. Wybierz liczbę pozycji i zacznijcie nową rundę.';
    } else {
      summaryLead.textContent = 'Wybraliśmy wszystkie wspólne typy. Czekamy w poczekalni, aż gospodarz wystartuje ponownie.';
    }
    return;
  }

  if (!selfApproved) {
    playAgainButton.disabled = rematchVoteSubmitting;
    playAgainButton.textContent = 'Gramy jeszcze raz?';
    if (approvals > 0) {
      summaryLead.textContent = 'Partner jest gotowy na kolejną rundę. Kliknij „Gramy jeszcze raz?”, aby do niego dołączyć.';
    } else {
      summaryLead.textContent = 'Wybraliśmy wszystkie wspólne typy. Kliknijcie „Gramy jeszcze raz?”, jeśli macie ochotę na kolejną rundę.';
    }
    return;
  }
  ...
  summaryLead.textContent = 'Czekamy na partnera. Gdy potwierdzi, przygotujemy dla Was nowe pozycje.';
}

function promptHostForRematch() {
  if (!isHost || !hostSetupCard) {
    rematchPromptedSessionId = null;
    return;
  }
  if (!rematchReady || !currentSession || !allFinished) {
    rematchPromptedSessionId = null;
    return;
  }
  const sessionId = currentSession.id;
  if (!sessionId || rematchPromptedSessionId === sessionId) {
    return;
  }
  rematchPromptedSessionId = sessionId;
  requestAnimationFrame(() => {
    hostSetupCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
}

async function submitRematchVote(triggerButton) {
  if (!currentSession || rematchVoteSubmitting || hasSelfApprovedRematch() || !allFinished) {
    return;
  }
  rematchVoteSubmitting = true;
  const button = triggerButton || playAgainButton;
  if (button) {
    button.disabled = true;
  }
  try {
    const payload = await postJson(rematchEndpoint, {
      room_key: roomKey,
...
function handleState(payload) {
  ...
  const voteEntries = Object.entries(payload.rematch_votes || {});
  rematchVotes = new Map(
    voteEntries.map(([id, value]) => [Number(id), Boolean(value)]),
  );
  rematchReady = Boolean(payload.rematch_ready);
  ...
  const shouldPinSetup = rematchReady && allFinished;
  forceSetupVisible = shouldPinSetup;
...
function initPlayAgain() {
  ...
  playAgainButton.addEventListener('click', () => {
    if (!currentSession || !allFinished || hasSelfApprovedRematch()) {
      return;
    }
    submitRematchVote(playAgainButton);
  });
}
