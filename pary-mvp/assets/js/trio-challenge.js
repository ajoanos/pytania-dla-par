...
function renderPlayers() {
  ...
  if (elements.turnLabel) {
    ...
    if (trio.winner) {
      if (trio.winner === 'draw') {
        statusText = 'Remis! Wylosujcie zadania.';
      } else {
        const winnerName = symbolName(trio.winner);
        statusText = winnerName ? `${winnerName} wygrał(a)!` : 'Gra zakończona.';
      }
    } else if (activeCount < 2) {
      statusText = 'Czekamy na graczy…';
      ...
      statusText = symbolOwner ? `Teraz ruch: ${symbolOwner} (${trio.currentSymbol})` : 'Kto zaczyna?';
    }
    const prefix = `Runda ${roundLabel}`;
    elements.turnLabel.textContent = statusText ? `${prefix} • ${statusText}` : prefix;
  }
}
...
function buildShareUrl() {
  ...
  const url = new URL('trio-challenge-room.html', window.location.href);
